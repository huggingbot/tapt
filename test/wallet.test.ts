...
describe('Wallet module', function () {
  ...
  describe('Count wallet scene', function () {
    ...
    it('should return the wallet count of 1 when wallets are present in the current chain', async function () {
      const wallet = Wallet.createRandom();
      const encryptedPrivateKey = encryptPrivateKey(wallet.privateKey);
      const network = ENetwork.Mainnet;
      sessionCtx.prop.chain.network = network;
      sessionCtx.prop.wallets[ENetwork.Mainnet].push({ encryptedPrivateKey, address: wallet.address, chainId: AppConfig[network].chainId });

      await scene.middleware()(ctx as IContext, jest.fn());
      expect(ctx.reply).toHaveBeenCalledWith('Your number of wallets is:\n1');
    });

    it('should return the wallet count of 0 when wallets are present in another chain', async function () {
      const wallet = Wallet.createRandom();
      const encryptedPrivateKey = encryptPrivateKey(wallet.privateKey);
      const network = ENetwork.Mainnet;
      sessionCtx.prop.chain.network = ENetwork.Local;
      sessionCtx.prop.wallets[ENetwork.Mainnet].push({ encryptedPrivateKey, address: wallet.address, chainId: AppConfig[network].chainId });
